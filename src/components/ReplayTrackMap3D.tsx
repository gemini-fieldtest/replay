...
const SceneContent: React.FC<SceneContentProps> = ({ positions, rotations, data, currentIndexRef, followMode, ghostPosition, showGhost, zoomLevel }) => {
  ...
  useFrame((_, delta) => {
    ...
    if (rotations && safeIdx < (rotations.length / 2)) {
         heading = rotations[safeIdx * 2 + 1]; // [pitch, yaw, ...]
    } else {
        if (safeIdx < maxIdx) {
...
export const ReplayTrackMap3D: React.FC<ReplayTrackMap3DProps> = memo(({ positions, rotations, data = [], currentIndexRef, currentIndex = 0, currentFrame = null, ghostPosition = null, showGhost, startLinePos }) => {
