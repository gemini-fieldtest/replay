...
const AnalogGauge: React.FC<AnalogGaugeProps> = ({
  ...
  for (let i = 0; i < numTicks; i++) {
    ...
    ticks.push(
      <g key={i}>
        <line x1={x1} y1={y1} x2={x2} y2={y2} stroke={theme === 'dark' ? "#4b5563" : "#cbd5e1"} strokeWidth="2" />
        <text x={tx} y={ty} textAnchor="middle" dominantBaseline="middle" className="text-[10px] fill-gray-500 dark:fill-gray-400 font-mono font-bold">
    ...
    <div className="flex flex-col items-center justify-center">
      ...
      <div className="relative" style={{ width: size, height: size }}>
        <svg width={size} height={size}>
          ...
          {/* Label */}
          <text x={center} y={center + 30} textAnchor="middle" className="fill-gray-500 dark:fill-gray-300 text-xs font-bold uppercase tracking-widest">
            {label}
          </text>
  ...
  return (
    <div className="w-full">
      <div className="flex justify-between mb-1">
        <span className="text-sm font-bold text-gray-700 dark:text-gray-200 tracking-wide">{label}</span>
        <div className="flex gap-3 font-mono">
            {ghostValue !== undefined && (
                 <span className="text-xs font-bold text-yellow-600 dark:text-yellow-500">{Math.round(ghostValue)}%</span>
            )}
            <span className="text-sm font-bold text-gray-900 dark:text-white">{Math.round(value)}%</span>
        </div>
      </div>
      <div className="w-full bg-gray-200 dark:bg-gray-900 rounded-full h-5 relative border border-gray-300 dark:border-gray-800">
        {/* Ghost Bar */}
  ...
  return (
    <div className="flex flex-col items-center">
      <div className="w-24 h-24 relative flex items-center justify-center">
        ...
        {/* Main Wheel */}
        <div
            className="w-full h-full rounded-full border-4 border-gray-400 dark:border-gray-500 absolute flex items-center justify-center transition-transform duration-100 ease-linear z-10 bg-gray-100/50 dark:bg-gray-900/50"
            style={{ transform: `rotate(${-angle}deg)` }}
        >
            {/* Spokes */}
            <div className="absolute w-full h-2 bg-gray-400 dark:bg-gray-500"></div>
            <div className="absolute h-full w-2 bg-gray-400 dark:bg-gray-500"></div>
            {/* Top Marker */}
            <div className="absolute top-0 w-2 h-4 bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.8)]"></div>
        </div>
      </div>
      <span className="mt-2 text-sm font-bold text-gray-600 dark:text-gray-300">STEERING</span>
    </div>
  );
};
...
const GForceMeter: React.FC<GForceMeterProps> = ({ lat, long, ghostLat, ghostLong, max = 2 }) => {
  ...
  return (
    <div className="w-24 h-24 bg-gray-100 dark:bg-gray-900 rounded-full border-2 border-gray-300 dark:border-gray-600 relative flex items-center justify-center overflow-hidden">
      {/* Crosshairs */}
      <div className="absolute w-full h-px bg-gray-300 dark:bg-gray-700"></div>
      <div className="absolute h-full w-px bg-gray-300 dark:bg-gray-700"></div>

      {/* Rings */}
      <div className="absolute w-12 h-12 rounded-full border border-gray-400/30 dark:border-gray-700/50"></div>
      <div className="absolute w-18 h-18 rounded-full border border-gray-400/20 dark:border-gray-700/30"></div>
...
const TelemetryGraph: React.FC<TelemetryGraphProps> = ({ data, ghostData, label, unit, color, min, max, currentValue }) => {
  ...
  return (
    <div className="flex flex-col bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-2 rounded min-w-[120px] justify-between shadow-sm">
      <div className="flex justify-between items-end mb-1">
        <span className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-tight">{label}</span>
        <span className="text-sm font-bold font-mono" style={{ color }}>
          {(currentValue ?? 0).toFixed(1)}{unit}
        </span>
      </div>

      <div className="flex-grow relative w-full h-10 overflow-hidden bg-gray-50 dark:bg-gray-950/30 rounded">
...
export const ReplayGauges: React.FC<ReplayGaugesProps> = ({ frame, getHistory, ghostFrame, getGhostHistory, isStacked = false }) => {
  ...
  return (
    <div className="bg-white/80 dark:bg-gray-950/80 p-6 rounded-lg border border-gray-200 dark:border-gray-800 flex flex-col gap-6 shadow-xl backdrop-blur-sm">
      ...
      <div className="flex justify-between items-center px-4">
        ...
        <div className={`flex items-center ${isStacked ? 'flex-row gap-12' : 'flex-col gap-4'}`}>
          ...
          <div className="flex flex-col items-center">
             ...
             />
             <div className="flex gap-2 mt-1 text-[10px] font-mono text-gray-500 dark:text-gray-400">
      ...
      <div className="flex gap-6 items-center px-12">
        ...
        <div className="flex-grow flex flex-col gap-2">
              <div className="flex justify-between items-end mb-1">
                 <span className="text-sm font-bold text-gray-700 dark:text-gray-200 tracking-wide">Brake</span>
                 <div className="flex gap-2 font-mono">
                     {ghostFrame && <span className="text-xs font-bold text-yellow-600 dark:text-yellow-500">{ghostFrame.brakePressure?.toFixed(1)} bar</span>}
                     <span className="text-sm font-bold text-gray-900 dark:text-gray-400">{frame.brakePressure?.toFixed(1) ?? '0.0'} bar</span>
                 </div>
              </div>
              <div className="w-full bg-gray-200 dark:bg-gray-900 rounded-full h-5 relative border border-gray-300 dark:border-gray-800">
             {ghostFrame && (
      ...
      <div className="flex gap-4 h-40">
        <div className="flex flex-col items-center bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 p-3 rounded h-full justify-center flex-1 min-w-[120px] shadow-sm">
          <div className="text-xs font-bold text-gray-500 dark:text-gray-400 mb-1 uppercase tracking-tight">Gear</div>
          <div className="text-5xl font-bold text-gray-900 dark:text-white mb-2 relative font-mono">
              {frame.gear === 0 ? 'N' : frame.gear}
              {ghostFrame && ghostFrame.gear !== frame.gear && (
                  <span className="absolute -top-1 -right-4 text-lg text-yellow-600 dark:text-yellow-500 font-bold font-mono">
